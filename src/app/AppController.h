...
public:
    ...
    void undo();
    void redo();
    bool can_undo() const { return cmd_mgr_.can_undo(); }
    bool can_redo() const { return cmd_mgr_.can_redo(); }
    ...
    size_t add_curve(const std::string& name, const std::vector<Vector>& control_points, int degree = 3);
    ...
    void add_control_point(size_t curve_index, const Vector& point, size_t position);

    // Add control point at end
    void add_control_point(size_t curve_index, const Vector& point);

    // Remove control point from curve
    void remove_control_point(size_t curve_index, size_t point_index);

    // Move control point
    void move_control_point(size_t curve_index, size_t point_index, const Vector& new_position);
